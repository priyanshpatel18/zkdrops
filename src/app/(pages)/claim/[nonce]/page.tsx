        ...
        <Card className="w-full shadow-lg">
            ...
            <div className="flex items-center justify-center">
              ...
              >
                {session?.campaign.tokenUri && (
                  <Image src={session.campaign.tokenUri} alt="Logo" fill className="object-cover" />
                )}
              </motion.div>
            </div>
            <CardTitle className="text-center text-xl">{session?.campaign.name || 'Claim Your NFT'}</CardTitle>
          ...
          <CardFooter className="pt-0 pb-4 px-4 flex flex-col">
            {error && status !== CLAIM_STATUSES.FAILED && status !== CLAIM_STATUSES.ERROR && (
              <div className="w-full mb-4 p-3 bg-red-50 text-red-600 text-sm rounded-md">{error}</div>
            )}
            {session && (
