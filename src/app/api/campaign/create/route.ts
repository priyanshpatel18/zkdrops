...
export async function POST(request: NextRequest) {
  ...
  if (!formData || typeof formData.tokenUri !== "string") return NextResponse.json({ error: "Invalid form data" }, { status: 400 });
  ...
  try {
    ...
    // Generate QR Code 
    const qrData = `https://zkdrops.xyz/claim?${campaign.id}`;
